<img width="630" height="630" alt="brainct_001" src="https://github.com/user-attachments/assets/c5e6a644-0b51-4f33-afaf-4bc3b5ba7eb1" />
...
<img width="220" height="220" alt="brainct_001" src=<<img width="630" height="630" alt="brainct_001" src="https://github.com/user-attachments/assets/7181df59-af19-497f-8b56-f7f7338630cf" />
...
- 8비트 BMP와 MEM(Verilog 메모리 초기화용) 파일로 결과 생성
...
- HLR-002: 색공간 변환
  - 소프트웨어는 표준 휘도 공식을 사용하여 RGB 픽셀을 그레이스케일로 변환해야 한다: Y = 0.299R + 0.587G + 0.114B
...
- LLR-003: 그레이스케일 변환 알고리즘
  - 각 픽셀에 대해 Y = 0.299R + 0.587G + 0.114B 공식 적용
  - 결과를 uint8_t 형으로 캐스팅
  - 변환된 값을 별도 배열에 저장
...
- 인증 담당자: ________________ 날짜: 2025년 8월 28일

# MEM(메모리 초기화 파일) 출력 지원 기능 추가
- 본 소프트웨어는 기존의 BMP 이미지 그레이스케일 변환 및 8비트 BMP 파일 생성 기능 이외에,
하드웨어 설계 및 검증 환경에서 직접 사용 가능한 MEM 파일(Verilog memory initialization file) 출력 기능을 추가 제공한다.

  - MEM 파일 동시 생성 : 변환된 그레이스케일 픽셀 데이터를 “output_image.mem” 파일에 2자리 16진수(00~FF)로 한 줄씩 줄바꿈(0x0D, 0x0A)하며 저장.

  - 활용 목적 : FPGA/ASIC 설계, RTL 시뮬레이션 등에서 $readmemh 등 메모리 초깃값 파일로 직접 활용 가능.

  - 연동성 강화 : BMP와 MEM 결과를 병렬로 제공, SW-하드웨어 데이터 연동 및 자동화 테스트 환경 구성이 용이.

  - 안정성 : MEM 파일 생성에서도 파일 입출력/메모리 에러 처리, 리소스 해제 등 기존 BMP 처리와 동일한 품질 및 신뢰성 보장.
